...
sim <- function(i){
  ...
  g0_known <- function(task){
    return(rep(0.5,task$nrow))
  }
  
  # define partially known likelihood
  factor_list <- list(define_lf(LF_emp, "W"), 
                      define_lf(LF_known, "A", mean_fun=g0_known, density_fun=g0_known), 
                      define_lf(LF_fit, "Y", learner = sim_learner_list[["Y"]], type = "mean"))
  likelihood_def <- Likelihood$new(factor_list)
  
  # Define spec:
  tmle_spec <- tmle3_Spec_mopttx_blip_revere$new(
    V = c("W1","W2","W3","W4"), type = "blip1",
    b_learner = sim_learner_list$B, maximize = TRUE, complex = TRUE, likelihood_override = likelihood_def
  )
  ...
  # tmle_task <- fit$tmle_task
  # cf_task <- fit$tmle_params[[1]]$cf_likelihood$cf_tasks[[1]]
  # Qa_init <- fit$likelihood$initial_likelihood$get_likelihood(tmle_task,"Y","validation")
  # Qdn_init <- fit$likelihood$initial_likelihood$get_likelihood(cf_task,"Y","validation")
  # A_is_dn = as.numeric(tmle_task$get_tmle_node("A")==cf_task$get_tmle_node("A"))
  # Y <- tmle_task$get_tmle_node("Y")
  # A <- tmle_task$get_tmle_node("A")
  # W <- tmle_task$get_tmle_node("W")
  # Qa0 <- Qbar0(A, as.matrix(W))
  # plot(Q0, Qa_init)
  # HA <- A_is_dn/0.5
  # Hdn <- rep(0.5,length(Y))
  # mean(Qa_init)
  # mean(Qdn_init)
  # IC_Q <- 0.5*mean(Y-Qa_init)
  # fluc <- glm(Y~ HA - 1, offset=plogis(Qa_init), family=binomial())
  # coef(fluc)
  # tmle_data <- data.table(Y,HA,Qa_init,Qdn_init,Hdn)
  # ggplot(tmle_data,aes(x=Qa_init,color=factor(Y)))+geom_density()
  est <- fit$summary
  ...
  save(results, file=sprintf("alexsim_results/results%04d.rdata",i))
  return(results)
}
...
save(results, file="alexsim_results.rdata")
