...
suboptimal_rule <- function(tmle_task){
  W <- tmle_task$get_tmle_node("W")
  W4 <- W[,4]
  return(as.numeric(W4>0.5))
}
...
tmle_spec <- tmle3_Spec_rule$new(rule=suboptimal_rule)
node_list <- list(W = c("W1", "W2", "W3", "W4"), A = "A", Y = "Y")
full_task <- tmle_spec$make_tmle_task(data_full,node_list)
data_full$tx <- suboptimal_rule(full_task)
psi<-mean(Qbar0(data_full$tx, as.matrix(full_task$get_tmle_node("W"))))
...
Q_learner <- Lrnr_sl$new(
  ...
  # learners = list(lrn1,lrnr_glm,true_para),
...
sim <- function(i){
  ...
  names(ests)<-c("type","param","init_est","tmle_est","se","lower","upper",
                "psi_transformed","lower_transformed","upper_transformed","coverage", "true_psi","iteration")
  
  results <- ests
  save(results, file=sprintf("sim1a_fixed_results/results%04d.rdata",i))
  return(results)
}
...
save(results, file="sim1a_fixed_results.rdata")