---
title: "Variable Importance with Optimal Individualized Categorical Treatment"
...
Many methods for learning an optimal rule from data have been developed. Here, we focus on the methods developed in @luedtke2016super and @vanderLaanLuedtke15; however `tmle3mopttx` also supports the widely used Q-learning approach, based on generating an estimate of $\bar{Q}_{Y,0}(A,W)$ @Sutton1998.

In particular, @luedtke2016super and @vanderLaanLuedtke15 outline a methodology for learning an optimal ITR using Super Learner @vdl2007super, and estimating its value using the cross-validated Targeted Minimum Loss-based Estimation (CV-TMLE) @cvtmle2010. Luedtke and van der Laan present three different appraches for leaning optimal rule, but `tmle3mopttx` relies on using the Super Learner to estimate the blip function (or "pseudo-blip" for categorical treatment). 
...
4. Inference for the mean outcome under the optimal rule using CV-TMLE.
...
`tmle3_mopttx`. We specify the argument `shift_grid = seq(-0.5, 0.5, by = 0.5)`
...
interest, we have specified the set of covariates the rule depends on ($V$), the type of pseudo-blip to use ("type"), and the learners used for estimating the pseudo-blip.

### Targeted Estimation of the Mean under the optimal ITR
